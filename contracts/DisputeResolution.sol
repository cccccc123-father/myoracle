...
contract DisputeResolution is IDisputeResolution {
    ...
    function close(uint256 requestId) external override onlyOwner {
        ...
        bool supportOriginal = (a.supportCount >= a.againstCount);

        bool unanimous = false;
        if (a.committee.length > 0 && totalVotes == a.committee.length) {
            if (a.supportCount == totalVotes || a.againstCount == totalVotes) {
                unanimous = true;
            }
        }

        emit AuditClosed(requestId, unanimous, supportOriginal);
    }
}
