...
contract NodeManager {
    ...
    // pick at most k addresses (fallback to observerCount when k == 0) meeting minStake
    function selectObservers(uint256 k, uint256 minStake) external view returns (address[] memory) {
        // k == 0 表示使用本合约里的默认 observerCount
        uint256 limit = k > 0 ? k : observerCount;
